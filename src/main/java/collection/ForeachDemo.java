...
public class ForeachDemo {
    public static void main(String[] args) {
        ...
        /*
            foreach
            Collection的foreach方法的要求传一个lambda表达式,然后将集合每个元素都
            顺序的传给lambda中的str,并执行一次表达式

            迭代器相比,由于迭代器是用另一个类Iterator来遍历集合,因此它遍历的过程中
            不允许用集合的方法增删元素(集合有并发安全的实现类,但是仍然不允许多线程操作时
            使用迭代器遍历与集合方法增删同时进行),否则会抛出异常.

            但是集合新添加的foreach方法在遍历的过程中没有上述限制,因为foreach是集合自身
            方法,因此对于并发安全的集合实现类,增删元素和foreach有很好的互斥性,可以保证多
            线程的并发安全.

            我们常用的集合实现类:ArrayList,LinkedList,HashSet都不是并发安全的,因此
            多线程情况下不应当操作他们
            集合的工具类:java.util.Collection提供一组静态方法,可以将现有的集合转换为
            一个并发安全的集合.
         */
        //将当前集合转换为一个线程安全的集合
